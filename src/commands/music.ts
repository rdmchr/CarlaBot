...
export class music {
    ...
    async seek(@SlashOption("time", {description: "The time to seek to in seconds.", type: "INTEGER"}) time: number, interaction: CommandInteraction): Promise<void> {
        const validate = this.validateInteraction(interaction);
        if (!validate) {
            return;
        }
    ...
    async volume(@SlashOption("volume", {description: "The volume to set.", type: "INTEGER"}) volume: number, interaction: CommandInteraction): Promise<void> {
        const validate = this.validateInteraction(interaction);
        if (!validate) {
            return;
        }
